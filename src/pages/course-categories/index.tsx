...
import { getCourseCategories, deleteCourseCategoryById } from 'apiSdk/course-categories';
...
function CourseCategoryListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('course_category', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/course-categories/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('course_category', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Course Category
            </Text>
            <NextLink href={`/course-categories/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('course_category', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/course-categories/edit/${record.id}`} passHref legacyBehavior>
                          <Button
