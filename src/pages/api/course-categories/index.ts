...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getCourseCategories() {
    const data = await prisma.course_category
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'course_category'));
    return res.status(200).json(data);
  }

  async function createCourseCategory() {
    await courseCategoryValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.course_category.create({
      data: body,
    });
    return res.status(200).json(data);
  }
